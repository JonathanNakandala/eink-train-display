...
font_time = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-ExtraLight.ttf'), 95)
font_direction = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-Light.ttf'), 23)
font_traininfo = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-Light.ttf'), 15)
font_dayofweek = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-SemiBold.ttf'), 65)
font_date = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-ExtraLight.ttf'), 65)
font_bigtemp = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-ExtraLight.ttf'), 75)
font_smalltemp = ImageFont.truetype(os.path.join(
    fontdir, 'Overpass/Overpass-ExtraLight.ttf'), 27)
...
def drawDepartures(draw, y, now, departures):
    ...
    for service in services:
        ...
        draw.text((train_columns[0], y), service.std,
                  font=font_traininfo, fill=0)
        draw.text((train_columns[1], y), service.destination.location[0].locationName.upper(
        ),  font=font_traininfo, fill=0)
        draw.text((train_columns[2], y), service.etd.upper(
        ),  font=font_traininfo, fill=0)
        draw.text((train_columns[3], y), time_until_text,
                  font=font_traininfo, fill=0)
        y += train_line_offset
